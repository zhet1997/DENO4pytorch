...
if __name__ == "__main__":
    ...
    ################################################################
    name = 'TNO'
    work_path = os.path.join('Demo', 'PakB_2d', '../work', name + '_' + str(3))
    ...
    for kk, hole_num in enumerate([1,2]):
        ...
        input = np.zeros([*design.shape[:-1],16])
    ...
    train_loader, valid_loader, x_normalizer, y_normalizer = get_loader_pakB(np.concatenate(train_input_list,
                                                                                            axis=0),
                                                                             np.concatenate(train_output_list,
                                                                                            axis=0),
                                                                             np.concatenate(valid_input_list,
                                                                                            axis=0),
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            ...
            for fig_id in range(5):
                fig, axs = plt.subplots(out_dim, 3, figsize=(18, 8), num=2)
                Visual.plot_fields_ms(fig, axs, train_true[fig_id], train_pred[fig_id], grids)
                fig.savefig(os.path.join(work_path, 'train_solution_' + str(fig_id) + '.jpg'))
                plt.close(fig)

            for fig_id in range(5):
                fig, axs = plt.subplots(out_dim, 3, figsize=(18, 8), num=3)
