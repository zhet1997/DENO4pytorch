...
class predictor(nn.Module):
    ...
    def forward(self, design, coords):
        ...
        # B = self.branch_net(design, pos=coords.unsqueeze(0))
        feature_list = []
        for ii in range(hole_num):
            feature_list.append(self.branch_net(design[ii]))
...
if __name__ == "__main__":
    ...
    ################################################################
    name = 'LSP'
    work_path = os.path.join('Demo', 'PakB_2d', '../../work', name + '_' + str(0))
    ...
    super_model = FcnSingle(planes=(config['n_targets'] * 2, 64, 64, config['n_targets']), last_activation=False).to(
        Device)
    config['node_feats'] = 1
    MLP_model = FourierTransformer(**config).to(Device)
    # Net_model = predictor(trunc=Tra_model, branch=MLP_model, field_dim=out_dim).to(Device)
    config['node_feats'] = config['n_targets']+1
    config['n_targets'] = out_dim
    Share_model = FourierTransformer(**config).to(Device)
