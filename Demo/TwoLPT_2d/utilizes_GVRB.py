...
def get_grid_interp1D(grid_num_s=64,
                    grid_num_z=256,
                    z=None,
                    hub_adjust=0.0001,
                    shroud_adjust=0.0001,
    ...
    if z is None:
        z = [-0.070, 0.015, 0.080, 0.155, 0.240]

    z_S1 = np.linspace(z[0]+0.0001, z[1]-0.0001, 64)
    z_R1 = np.linspace(z[1], z[2]-0.0001, 65)
    z_S2 = np.linspace(z[2], z[3]-0.0001, 65)
    z_R2 = np.linspace(z[3], z[4]-0.0001, 65)
    x = np.concatenate((z_S1, z_R1[1:], z_S2[1:], z_R2[1:]), axis=0)
    xx = np.tile(x, [grid_num_s, 1])

    y = np.linspace(0.370+hub_adjust, 0.490-shroud_adjust, 64)
    yy = np.tile(y, [grid_num_z, 1]).T
    return np.concatenate([xx[:,:,np.newaxis],yy[:,:,np.newaxis]],axis=2)
...
def get_origin(quanlityList=None,
    ...
    if realpath is None:
        ...
        if type == 'struct':
            sample_files = [os.path.join(realpath, 'TwoLPT2000'),
                            ]
        elif type == 'unstruct':
            ...
            assert False
        geom_files = [os.path.join(realpath, "sampleGVM_2000")
                      ]
...
class GVRBWeightLoss(torch.nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        device = target.device
        if target.shape[1] > 4000:
            target = torch.reshape(target, (target.shape[0], 64, 256, -1))
            predicted = torch.reshape(predicted, (target.shape[0], 64, 256, -1))
        ...
        temp_RS = torch.ones((grid_size_1, self.weighted_lines+3), dtype=torch.float32, device=device) * self.weighted_cof
        temp_S = torch.ones((grid_size_1, int(self.RS_position-self.weighted_lines*2)), dtype=torch.float32, device=device)
        temp_R = torch.ones((grid_size_1, int(self.RS_position-self.weighted_lines*2)), dtype=torch.float32,
                            device=device)
        weighted_mat = torch.cat((temp_LT, temp_S, temp_RS, temp_R, temp_RS, temp_S, temp_RS, temp_R, temp_LT), dim=1)
        weighted_mat = weighted_mat.unsqueeze(0).unsqueeze(-1).expand_as(target)
        weighted_mat = weighted_mat * grid_size_2 /(self.weighted_cof * (self.weighted_lines *8)
                                                    + grid_size_2 - (self.weighted_lines *8))
