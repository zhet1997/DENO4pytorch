...
def plot_spanwise_distribution(rst_true, rst_pred, num, variable_name, save_name, save_path):
    ...
    if np.min(rst_true)>=0.5 and np.max(rst_true)<=1:
        xlim_min, xlim_max = np.min(rst_true)-0.1, 1.01
    elif np.min(rst_true)<=0 and np.max(rst_true)<=0.5:
        xlim_min, xlim_max = np.min(rst_true)-0.1, 0.501
    else:
        xlim_min, xlim_max = np.min(rst_true)*0.95, np.max(rst_true)*1.05
    ...
    plt.ylim(-0.01, 1.01)
    plt.legend(prop=font1, frameon=False, loc='center left')
    ax00 = plt.gca()
    bwith = 1.0
...
if __name__ == '__main__':
    ...
    name='TNO_9'
    pathNpz = os.path.join('E:\WQN\CODE\DENO4pytorch\Demo\GVRB_2d\work\{}\save_valid.npz'.format(name))
    ...
    #比较沿叶高分布
    # variable_name_list = ['Total_total_efficiency', 'Total_static_efficiency', 'Static_pressure_ratio','Static_Enthalpy']
    # save_path = "E://WQN//CODE//DENO4pytorch//Demo//GVRB_2d//work//{}//result_spanwised".format(name)
    ...
    # save_path = "E://WQN//CODE//DENO4pytorch//Demo//GVRB_2d//work//{}//result_averaged".format(name)
    # if not os.path.exists(save_path):
    #     os.makedirs(save_path)
    # for variable_name in variable_name_list:
    ...
    #                  'Absolute Total Temperature': 7,
    #                  }
    # save_path = "E://WQN//CODE//DENO4pytorch//Demo//GVRB_2d//work//{}//result_basic".format(name)
    # if not os.path.exists(save_path):
    #     os.makedirs(save_path)
    # Visual = MatplotlibVision(save_path, input_name=('x', 'y'), field_name=('Static Pressure','Static Temperature','Density','Vx','Vy','Vz',
    #                  'Relative Total Temperature', 'Absolute Total Temperature'))
    # for fig_id in range(5):
    #     fig, axs = plt.subplots(8, 3, figsize=(18 ,20), num=2)
    #     Visual.plot_fields_ms(fig, axs, data_true[fig_id], data_pred[fig_id], grids)
    #     fig.savefig(os.path.join(save_path, 'solution_' + str(fig_id) + '.jpg'))
    #     plt.close(fig)

    # 比较子午流面流场云图预测结果(衍生物理场)
    # variable_name_list =['Absolute Total Pressure',
    #                         'Absolute Total Temperature','Relative Mach Number','Static Enthalpy']
    # derive_field_true = np.zeros((data_true.shape[0], 64, 128, len(variable_name_list)))
    # derive_field_pred = np.zeros((data_true.shape[0], 64, 128, len(variable_name_list)))
    # for ii, variable_name in enumerate(variable_name_list):
    #     rst_true = post_true.get_field('{}'.format(variable_name))
    #     print(rst_true.shape)
    #     derive_field_true[:,:,:,ii] = rst_true
    #
    #     rst_pred = post_pred.get_field('{}'.format(variable_name))
    #     print(rst_pred.shape)
    #     derive_field_pred[:, :, :, ii] = rst_pred
    #
    # save_path = "E://WQN//CODE//DENO4pytorch//Demo//GVRB_2d//work//{}//result_detive".format(name)
    # if not os.path.exists(save_path):
    #     os.makedirs(save_path)
    # Visual = MatplotlibVision(save_path, input_name=('x', 'y'), field_name=('Absolute Total Pressure',
    #                         'Absolute Total Temperature','Relative Mach Number','Static Enthalpy'))
    # for fig_id in range(5):
    #     fig, axs = plt.subplots(len(variable_name_list), 3, figsize=(18 ,10), num=2)
    #     Visual.plot_fields_ms(fig, axs, derive_field_true[fig_id], derive_field_pred[fig_id], grids)
    #     fig.savefig(os.path.join(save_path, 'solution_' + str(fig_id) + '.jpg'))
    #     plt.close(fig)

    # 反动度沿叶高分布
    variable_name_list = ['Degree_reaction']
    save_path = "E://WQN//CODE//DENO4pytorch//Demo//GVRB_2d//work//{}//result_spanwised".format(name)
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    for variable_name in variable_name_list:
        rst_true = post_true.get_performance('{}'.format(variable_name), computing_station=3, type='spanwised')
        print(rst_true.shape)

        rst_pred = post_pred.get_performance('{}'.format(variable_name), computing_station=3, type='spanwised')
        print(rst_pred.shape)
