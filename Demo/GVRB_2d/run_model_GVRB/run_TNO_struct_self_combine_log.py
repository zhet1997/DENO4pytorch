...
import time
import yaml
from Demo.GVRB_2d.utilizes_GVRB import get_origin, SelfSuperviseLoss, SelfSuperviseLoss2
...
def train_combine(dataloader_1, dataloader_2,
                  netmodel, device,
                  lossfunc_1, lossfunc_2,
    ...
    scheduler.step()
    return train_loss_1 / (batch + 1), train_loss_2 / (batch + 1)
...
def generate_virtual_loader(x_normalizer, virtual_batchs, batch_size):
    ...
    data_virtual = x_normalizer.sample_generate(virtual_batchs, 4, norm=False)
    post = cfdPost_2d()
    post.bouCondition_data_readin(
        boundarycondition=data_virtual[:, -4:],
        # x_norm=x_normalizer_bc,
    )
    field_matrix, bc_matrix = post.get_dimensional_matrix(expand=batch_size, scale=[-0.02, 0.02])
    ...
    data_expand = post.data_expand(data_virtual, expand=batch_size, keeporder=True)
...
if __name__ == "__main__":
    ...
    name = 'TNO'
    work_path = os.path.join('../work', name + '_' + str(21) + '_self_combine_log')
    ...
    x_normalizer_bc = DataNormer(train_x.numpy(), method='log')
    ...
    Loss_self = SelfSuperviseLoss2()
    # # Loss_func = nn.SmoothL1Loss()
    # # 优化算法
    Optimizer = torch.optim.Adam(Net_model.parameters(), lr=learning_rate, betas=(0.7, 0.9))#, weight_decay=1e-7)
    Optimizer_self = torch.optim.Adam(Net_model.parameters(), lr=learning_rate, betas=(0.7, 0.9))  # , weight_decay=1e-7)
    # # 下降策略
    ...
    for epoch in range(epochs):
        ...
        loss1, loss2 = train_combine(train_loader, self_loader,
                                         Net_model, Device,
                                         Loss_func, Loss_self, Optimizer_self, Scheduler,
                                         y_norm=y_normalizer)
        del self_loader
