...
def train(dataloader, netmodel, device, lossfunc, optimizer, scheduler):
    ...
    train_loss = 0
    for batch, (xx, yy, gg) in enumerate(dataloader):
        xx = xx.to(device)
        yy = yy.to(device)
        gg = gg.to(device)
        coords = gg#grid.tile([xx.shape[0], 1, 1, 1])
...
if __name__ == "__main__":
    ...
    name = 'TNO'
    work_path = os.path.join('../work', name + str(0))
    ...
    g_normalizer.save(work.g_norm)


    ################################################################
    #  Neural Networks
    ################################################################
    with open(os.path.join('../data/configs/transformer_config_gvrb_simple.yml')) as f:
