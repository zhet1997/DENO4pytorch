...
if __name__ == "__main__":
    ...
    name = 'TNO'
    work_path = os.path.join('../work', name + '_' + str(8))
    ...
    design, fields, grids = get_origin(type='struct', quanlityList = ["Static Pressure", "Static Temperature", "Density",
                                                        "Vx", "Vy", "Vz"])  # 获取原始数据取原始数据
    ...
    post = cfdPost_2d()
    valid_loader_sim_1 = post.loader_similarity(valid_loader, grid=grids, scale=[-0.02, 0.02], expand=1, log=True, inputdict=inputdict)
    valid_loader_sim_2 = post.loader_similarity(valid_loader, grid=grids, scale=[-0.05, 0.05], expand=1, log=True, inputdict=inputdict)
    valid_loader_sim_3 = post.loader_similarity(valid_loader, grid=grids, scale=[-0.1, 0.1], expand=1, log=True, inputdict=inputdict)
    del post

    for epoch in range(epochs):
        # if epoch<500:
        #     scale_value = (np.log10(1+(np.power(10,0.08)-1)*(epoch)/500))
        # else:
        #     scale_value = (np.log10(1 + (np.power(10, 0.08) - 1) * (1000-epoch) / 500))

        # if epoch % 50 == 0:
        #     print(scale_value)
        #     post = cfdPost_2d()
        #     train_loader_sim = post.loader_similarity(train_loader, grid=grids, scale=[-scale_value, scale_value], expand=2, log=True)
        #     del post
        ...
        if epoch==epochs-1:
            torch.save({'log_loss': log_loss, 'net_model': Net_model, 'optimizer': Optimizer.state_dict()},
                       os.path.join(work_path, 'final_model.pth'))
