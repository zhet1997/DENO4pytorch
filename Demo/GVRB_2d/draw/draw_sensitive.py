...
def draw_sensitive_01d(predictor, work=None):
    evaluater = lambda x: predictor.predictor_cfd_value(x, input_norm=False, parameterList=parameterList, grid=grid,
                                                        space=1)
    match_dict = get_match_dict()
    # # temp = [x for x in match_dict.keys() if not '_' in x]
    for var_name in match_dict.keys():
        ...
        for j in range(len(parameterList)):
            ...
            Visual.plot_value_std_clean(fig, axs, mean[:, j], np.linspace(0, 1, mean.shape[0]),
                                        ...
                                        xylabels=('x', 'y'), rangeIndex=20, color='darkgreen')
            fig.savefig(os.path.join(save_path, 'spanStd_' + var_name + '_' + parameterList[j] + '.jpg'))
            plt.close(fig)
...
if __name__ == '__main__':
    ...
    # draw_sensitive_0d(predictor, work=work)

    evaluater = lambda x: predictor.predictor_cfd_value(x, input_norm=False, parameterList=parameterList, grid=grid, space=2)
    match_dict = get_match_dict()
    # # temp = [x for x in match_dict.keys() if not '_' in x]
    for var_name in match_dict.keys():
        ## get the draw data
