...
if __name__ == "__main__":
    ...
    ################################################################
    name = 'FNO'
    work_path = os.path.join('work', name + '_' + str(4) + '_self_combine_reg_5.3.3_expand_2')
    ...
    virtual_batch_size = batch_size * 3
    ...
    Loss_reg = SelfSuperviseLoss_reg(torch.as_tensor(delta,dtype=torch.float, device=Device))
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            self_loader, simlar_loader = generate_virtual_loader_new(x_normalizer, virtual_batchs, virtual_batch_size)
