...
class cfdPost_2d(object):
    ...
    def get_field_calculate_dict(self):
        ...
        self.fieldCalculateDict['Absolute Mach Number'] = lambda x1, x2: np.sqrt(self. x1 / x2)
        self.fieldParaDict['Absolute Mach Number'] = ('|V|^2', '|Speed Of Sound|^2')

        self.fieldCalculateDict['Relative Mach Number'] = lambda x1, x2: np.sqrt(self. x1 / x2)
        ...
        if nodeList is not None:
            ...
            if zList is not None:
                ...
                for ii, z in enumerate(zList):
                    tmp = (self.gird[:,:,0] < z).astype(int)
                    idxList[ii] = np.where(tmp==0)[0]
                    weightList[ii] = (z - self.gird[idxList[ii],0,0]) / (self.gird[idxList[ii+1],0,0] - self.gird[idxList[ii],0,0])
