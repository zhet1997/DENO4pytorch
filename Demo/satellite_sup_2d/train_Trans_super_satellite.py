...
from Demo.satellite_sup_2d.trains_satellite import (
    train_supercondition, valid_supercondition, valid_detail, supredictor_list_windows
)


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--h5', type=str, default=None, help='H5 数据路径, 默认使用utilizes_satellite内置路径')
...
if __name__ == "__main__":
    ...
    # 网络
    pred_model = FourierTransformer(**pred_model_dict).to(Device)
    super_model = FNO2d(in_dim=2, out_dim=1, **super_model_dict).to(Device)
    Net_model = supredictor_list_windows(pred_model, super_model, channel_num=meta['in_dim'], win_split=args.win_split).to(Device)

    # 损失与优化器
    Loss_func_train = nn.MSELoss()
    Loss_func_valid = nn.MSELoss()
    Optimizer_C = torch.optim.Adam(Net_model.pred_net.parameters(), lr=learning_rate, betas=(0.7, 0.9), weight_decay=1e-7)
    Optimizer_CS = torch.optim.Adam([
        {'params': Net_model.pred_net.parameters(), 'lr': learning_rate * 0.1},
        {'params': Net_model.super_net.parameters()},
    ], lr=learning_rate, betas=(0.7, 0.9), weight_decay=1e-7)
    Scheduler_C = torch.optim.lr_scheduler.StepLR(Optimizer_C, step_size=scheduler_step, gamma=scheduler_gamma)
    Scheduler_CS = torch.optim.lr_scheduler.StepLR(Optimizer_CS, step_size=scheduler_step, gamma=scheduler_gamma)
    ...
    for epoch in range(epochs):
        ...
        log_loss['train_c_loss'].append(
            train_supercondition(train_loader, Net_model, Device, Loss_func_train, Optimizer_C, Scheduler_C,
                                 x_norm=x_normalizer, super_num=0, channel_num=meta['in_dim'])
        )
        log_loss['train_cs_loss'].append(
            train_supercondition(train_loader, Net_model, Device, Loss_func_train, Optimizer_CS, Scheduler_CS,
                                 x_norm=x_normalizer, super_num=1, channel_num=meta['in_dim'])
        )

        Net_model.eval()
        if valid_loader is not None:
            log_loss['valid_c_loss'].append(
                valid_supercondition(valid_loader, Net_model, Device, Loss_func_valid,
                                     x_norm=x_normalizer, super_num=0, channel_num=meta['in_dim'])
            )
            log_loss['valid_cs_loss'].append(
                valid_supercondition(valid_loader, Net_model, Device, Loss_func_valid,
                                     x_norm=x_normalizer, super_num=1, channel_num=meta['in_dim'])
            )
        ...
        if epoch % 10 == 0:
            ...
            Visual.plot_loss(fig, axs, np.arange(len(log_loss['train_cs_loss'])), np.array(log_loss['train_cs_loss']), 'train_cs')
            if len(log_loss['valid_c_loss']) > 0:
                ...
                Visual.plot_loss(fig, axs, np.arange(len(log_loss['valid_cs_loss'])), np.array(log_loss['valid_cs_loss']), 'valid_cs')
        ...
        print('epoch: {:6d}, lr: {:.3e}, train_c: {:.3e}, train_cs: {:.3e}, cost: {:.2f}'.format(
            epoch,
            Optimizer_C.state_dict()['param_groups'][0]['lr'],
            log_loss['train_c_loss'][-1],
            log_loss['train_cs_loss'][-1],
