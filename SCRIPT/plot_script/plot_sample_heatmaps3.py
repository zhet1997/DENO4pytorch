...
样本对比云图绘制脚本（双模型对比版）
功能：绘制 N行6列 的对比图
每行：[布局图] [真实温度] [模型1预测] [模型1误差] [模型2预测] [模型2误差]
所有参数集中在顶部配置区
"""
...
def plot_sample_heatmaps(true_data: np.ndarray,
    ...
    绘制样本对比云图：2*N 行 4 列
    每个样本用两行表示：
        上行：[布局图] [真实温度] [模型1预测] [模型1误差]
        下行：[布局图] [真实温度] [模型2预测] [模型2误差]
    """
    ...
    temp_vmin = TEMP_VMIN if TEMP_VMIN is not None else min(
        true_data.min(), pred_data_model1.min(), pred_data_model2.min()
    )
    temp_vmax = TEMP_VMAX if TEMP_VMAX is not None else max(
        true_data.max(), pred_data_model1.max(), pred_data_model2.max()
    )

    # 计算全局误差范围（两个模型的误差）
    all_errors_model1 = pred_data_model1 - true_data
    all_errors_model2 = pred_data_model2 - true_data
    error_vmax = ERROR_VMAX if ERROR_VMAX is not None else max(
        np.abs(all_errors_model1).max(), np.abs(all_errors_model2).max()
    )

    print(f"温度范围: [{temp_vmin:.2f}, {temp_vmax:.2f}]")
    print(f"误差范围: [-{error_vmax:.2f}, {error_vmax:.2f}]")

    # 创建子图网格：2*n_rows x 4 列
    ...
    # 万一只有一个样本，axes 依然是 2D（2 行 4 列），不用 reshape
    ...
    for i, idx in enumerate(sample_indices):
        ...
        # 提取温度和误差数据
        true_field = true_data[idx, :, :, 0]        # (H, W)
        pred_field_model1 = pred_data_model1[idx, :, :, 0]
        pred_field_model2 = pred_data_model2[idx, :, :, 0]
        error_field_model1 = pred_field_model1 - true_field
        error_field_model2 = pred_field_model2 - true_field

        # 小工具函数：在指定行画 布局 + 真值
        def plot_layout_and_true(row_idx: int):
            # 布局图
            ax_layout = axes[row_idx, 0]
            if os.path.exists(json_path):
                plot_layout_on_ax(ax_layout, json_path)
            else:
                ax_layout.text(
                    0.5, 0.5, f'JSON not found:\n{sample_folder}',
            ...
            im = ax_true.imshow(
                true_field, cmap=TEMP_CMAP, origin='lower',
                vmin=temp_vmin, vmax=temp_vmax
            )
            ax_true.contour(
                true_field, levels=contour_levels,
                colors=TEMP_CONTOUR_COLOR,
                linewidths=TEMP_CONTOUR_LINEWIDTH,
                alpha=TEMP_CONTOUR_ALPHA,
                origin='lower'
            )
            ax_true.axis('off')

        # 上下两行都画一遍 布局 + 真值（信息稍微冗余但阅读方便）
        plot_layout_and_true(row_top)
        plot_layout_and_true(row_bottom)

        # -------- 上行：模型1 --------
        # 模型1预测
        ax = axes[row_top, 2]
        ax.imshow(
            pred_field_model1, cmap=TEMP_CMAP, origin='lower',
            vmin=temp_vmin, vmax=temp_vmax
        )
        ax.contour(
            pred_field_model1, levels=contour_levels,
            colors=TEMP_CONTOUR_COLOR,
            linewidths=TEMP_CONTOUR_LINEWIDTH,
            alpha=TEMP_CONTOUR_ALPHA,
            origin='lower'
        ...
        ax = axes[row_top, 3]
        ax.imshow(
            error_field_model1, cmap=ERROR_CMAP,
            origin='lower', norm=error_norm
        )
        ax.axis('off')

        # -------- 下行：模型2 --------
        # 模型2预测
        ax = axes[row_bottom, 2]
        ax.imshow(
            pred_field_model2, cmap=TEMP_CMAP, origin='lower',
            vmin=temp_vmin, vmax=temp_vmax
        )
        ax.contour(
            pred_field_model2, levels=contour_levels,
            colors=TEMP_CONTOUR_COLOR,
            linewidths=TEMP_CONTOUR_LINEWIDTH,
            alpha=TEMP_CONTOUR_ALPHA,
            origin='lower'
        ...
        # 模型2误差
        ax = axes[row_bottom, 3]
        ax.imshow(
            error_field_model2, cmap=ERROR_CMAP,
            origin='lower', norm=error_norm
        )
        ax.axis('off')
...
def main():
    ...
    # 绘制并保存（保存到模型1目录）
    save_path = os.path.join(EVAL_DIR_MODEL1, f'sample_heatmaps_compare_{SPLIT}_new2.png')
