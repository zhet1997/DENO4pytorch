...
# 输入参数
MODEL_TYPE = 'mlp'  # 模型类型：'fno' 或 'mlp'
SAMPLE_NUM = 4000    # 样本数量：100, 500, 1000, 2000, 4000
...
# 线条样式
LINEWIDTH_BASE = 2.5    # BASE方法线宽
LINEWIDTH_DSSL = 2.5    # DSSL方法线宽
LINESTYLE_BASE = '-'   # BASE方法线型（虚线）
LINESTYLE_DSSL = '-'    # DSSL方法线型（实线）
ALPHA_BASE = 0.4        # BASE方法透明度（0-1，1为不透明）
ALPHA_DSSL = 1.0        # DSSL方法透明度（1为不透明）

# 配色方案（按loss类型统一颜色，通过线型区分BASE/DSSL）
COLORS = {
    'base_train': '#1f77b4',      # BASE train：蓝色（虚线）
    'base_valid': '#FFC107',      # BASE valid：黄色（虚线）
    'base_train_self': '#2ca02c', # BASE train_self：绿色（虚线）
    'dssl_train': '#1f77b4',      # DSSL train：蓝色（实线）
    'dssl_valid': '#FFC107',      # DSSL valid：黄色（实线）
    'dssl_train_self': '#2ca02c'  # DSSL train_self：绿色（实线）
}
...
MARKER_INTERVAL = 50          # Marker间隔（每N个epoch显示一个marker）
MARKER_BASE = '^'             # BASE方法marker样式（三角形）
MARKER_DSSL = 's'             # DSSL方法marker样式（方块）
MARKER_SIZE = 12               # Marker大小
MARKER_EDGE_WIDTH = 1.5       # Marker边框宽度
MARKER_FACE_COLOR = 'black'   # Marker内部填充颜色
...
OUTPUT_FILENAME = f'convergence_{MODEL_TYPE}_n{SAMPLE_NUM}.png'  # 输出文件名
...
def plot_paper_convergence():
    ...
    # 绘制DSSL方法的3条线（实线 + 不透明 + 方块marker）
    for loss_type in ['train', 'valid', 'train_self']:
        values = dssl_data[loss_type]
        if len(values) > 0:
            epochs = np.arange(len(values))
            color_key = f'dssl_{loss_type}'
            
            # 绘制主曲线（不带label，图例由后面的虚拟线条生成）
            ax.semilogy(
                epochs, values,
                color=COLORS[color_key],
                linestyle=LINESTYLE_DSSL,
                linewidth=LINEWIDTH_DSSL,
                alpha=ALPHA_DSSL
            )
