...
# --- 数据路径配置 ---
EVAL_DIR_MODEL1 = '/data/wqn/DENO4pytorch/work_post_eval/work_satellite_dssl_fno_20251021/FNO_DSSL_n4000_20251025_014123'  # 模型1评估结果目录
EVAL_DIR_MODEL2 = '/data/wqn/DENO4pytorch/work_post_eval/work_satellite_BASE_fno_20251024/FNO_BASE_n2000_20251025_034709'  # 模型2评估结果目录
...
SPECIFIED_INDICES = [0, 10, 17, 30, 40, 50]  # 指定索引（相对于当前split）

# --- 图像尺寸与分辨率 ---
FIG_WIDTH = 8              # 图像宽度（英寸，2×3布局更紧凑）
FIG_HEIGHT_PER_SAMPLE = 4.5 # 每个样本（两行）高度（英寸）
DPI = 150                   # 分辨率
...
def map_split_index_to_global(split_idx: int, split: str, _ntrain: int, nvalid: int, total_samples: int) -> str:
    """
    ...
    Args:
        ...
        split: 'train' 或 'valid'
        _ntrain: 训练集样本数（保留参数以保持接口一致性，未使用）
        nvalid: 验证集样本数
...
def plot_sample_heatmaps(true_data: np.ndarray,
                         pred_data_model1: np.ndarray,
                         pred_data_model2: np.ndarray,
                         ...
                         save_path: str):
    """
    绘制样本对比云图：每个样本使用 2 行 × 3 列布局
    上行：[布局 | 模型1预测 | 模型1误差]；下行：[真实温度 | 模型2预测 | 模型2误差]
    """
    n_samples = len(sample_indices)
    
    # 计算全局温度范围（包含真实值和两个模型的预测值）
    temp_vmin = TEMP_VMIN if TEMP_VMIN is not None else min(true_data.min(), pred_data_model1.min(), pred_data_model2.min())
    temp_vmax = TEMP_VMAX if TEMP_VMAX is not None else max(true_data.max(), pred_data_model1.max(), pred_data_model2.max())
    
    # 计算全局误差范围（两个模型的误差）
    all_errors_model1 = pred_data_model1 - true_data
    all_errors_model2 = pred_data_model2 - true_data
    error_vmax = ERROR_VMAX if ERROR_VMAX is not None else max(np.abs(all_errors_model1).max(), np.abs(all_errors_model2).max())
    
    print(f"温度范围: [{temp_vmin:.2f}, {temp_vmax:.2f}]")
    print(f"误差范围: [-{error_vmax:.2f}, {error_vmax:.2f}]")
    
    total_rows = n_samples * 2
    fig, axes = plt.subplots(total_rows, 3, figsize=(FIG_WIDTH, FIG_HEIGHT_PER_SAMPLE * n_samples))
    if total_rows == 1:
        axes = axes.reshape(1, -1)
    
    contour_levels = np.linspace(temp_vmin, temp_vmax, TEMP_CONTOUR_LEVELS)
    error_norm = mcolors.TwoSlopeNorm(vmin=-error_vmax, vcenter=0, vmax=error_vmax)
    ...
    for i, idx in enumerate(sample_indices):
        ...
        pred_field_model1 = pred_data_model1[idx, :, :, 0]  # (64, 64)
        pred_field_model2 = pred_data_model2[idx, :, :, 0]  # (64, 64)
        error_field_model1 = pred_field_model1 - true_field
        error_field_model2 = pred_field_model2 - true_field
        
        top_row = 2 * i
        bottom_row = top_row + 1
        
        # 上行：布局图
        ax_layout = axes[top_row, 0]
        if os.path.exists(json_path):
            plot_layout_on_ax(ax_layout, json_path)
        else:
            ax_layout.text(0.5, 0.5, f'JSON not found:\n{sample_folder}',
                           ha='center', va='center', transform=ax_layout.transAxes)
            ax_layout.axis('off')
        
        # 上行：模型1预测
        ax_pred1 = axes[top_row, 1]
        ax_pred1.imshow(pred_field_model1, cmap=TEMP_CMAP, origin='lower',
                        vmin=temp_vmin, vmax=temp_vmax)
        ax_pred1.contour(pred_field_model1, levels=contour_levels, colors=TEMP_CONTOUR_COLOR,
                         linewidths=TEMP_CONTOUR_LINEWIDTH, alpha=TEMP_CONTOUR_ALPHA, origin='lower')
        ax_pred1.axis('off')
        
        # 上行：模型1误差
        ax_err1 = axes[top_row, 2]
        ax_err1.imshow(error_field_model1, cmap=ERROR_CMAP, origin='lower', norm=error_norm)
        ax_err1.axis('off')
        
        # 下行：真实温度
        ax_true = axes[bottom_row, 0]
        ax_true.imshow(true_field, cmap=TEMP_CMAP, origin='lower',
                       vmin=temp_vmin, vmax=temp_vmax)
        ax_true.contour(true_field, levels=contour_levels, colors=TEMP_CONTOUR_COLOR,
                        linewidths=TEMP_CONTOUR_LINEWIDTH, alpha=TEMP_CONTOUR_ALPHA, origin='lower')
        ax_true.axis('off')
        
        # 下行：模型2预测
        ax_pred2 = axes[bottom_row, 1]
        ax_pred2.imshow(pred_field_model2, cmap=TEMP_CMAP, origin='lower',
                        vmin=temp_vmin, vmax=temp_vmax)
        ax_pred2.contour(pred_field_model2, levels=contour_levels, colors=TEMP_CONTOUR_COLOR,
                         linewidths=TEMP_CONTOUR_LINEWIDTH, alpha=TEMP_CONTOUR_ALPHA, origin='lower')
        ax_pred2.axis('off')
        
        # 下行：模型2误差
        ax_err2 = axes[bottom_row, 2]
        ax_err2.imshow(error_field_model2, cmap=ERROR_CMAP, origin='lower', norm=error_norm)
        ax_err2.axis('off')
...
def main():
    print("=" * 60)
    print("样本对比云图绘制脚本（双模型对比版）")
    print(f"数据集划分: {SPLIT}")
    print(f"训练集: {NTRAIN}, 验证集: {NVALID}")
    print(f"模型1路径: {EVAL_DIR_MODEL1}")
    print(f"模型2路径: {EVAL_DIR_MODEL2}")
    print("=" * 60)
    
    # 加载模型1预测数据
    pred_data_model1, true_data = load_prediction_data(EVAL_DIR_MODEL1, SPLIT)
    
    # 加载模型2预测数据（使用模型1的true_data，确保一致）
    pred_data_model2, true_data_model2 = load_prediction_data(EVAL_DIR_MODEL2, SPLIT)
    
    # 验证两个模型的真实值数据一致
    assert np.array_equal(true_data, true_data_model2), "两个模型的真实值数据不一致"
    ...
    # 验证数据形状一致
    assert pred_data_model1.shape[0] == true_data.shape[0], \
        f"模型1数据样本数不一致: pred={pred_data_model1.shape[0]}, true={true_data.shape[0]}"
    assert pred_data_model2.shape[0] == true_data.shape[0], \
        f"模型2数据样本数不一致: pred={pred_data_model2.shape[0]}, true={true_data.shape[0]}"
    
    split_samples = true_data.shape[0]
    ...
    # 绘制并保存（保存到模型1目录）
    save_path = os.path.join(EVAL_DIR_MODEL1, f'sample_heatmaps_compare_{SPLIT}_new3.png')
    plot_sample_heatmaps(true_data, pred_data_model1, pred_data_model2, sample_indices, 
