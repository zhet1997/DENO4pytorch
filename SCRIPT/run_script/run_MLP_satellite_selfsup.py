...
if __name__ == "__main__":
    ...
    # DataLoader使用2D格式
    train_loader = DataLoader(TensorDataset(train_x_2d, train_y_2d), batch_size=args.batch_size, shuffle=False, drop_last=False)
    valid_loader = DataLoader(TensorDataset(valid_x_2d, valid_y_2d), batch_size=args.batch_size, shuffle=False, drop_last=False)
    ...
    Scheduler = torch.optim.lr_scheduler.StepLR(Optimizer, step_size=int(args.epochs*0.2), gamma=0.5)

    # 自监督数据加载（不含 temperature），保持2D格式
    ...
    # 转为tensor并归一化（保持2D格式）
    ss_inputs_2d = torch.tensor(ss_inputs_np, dtype=torch.float32)  # (N, s, s, 6)
