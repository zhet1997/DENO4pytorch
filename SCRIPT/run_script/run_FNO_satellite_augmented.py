...
from Demo.satellite_2d_dssl.dataset_augment import prepare_augmented_dataloader
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cuda_index', type=int, default=4)
    ...
    parser.add_argument('--config_path', type=str, 
                        default=os.path.join(PROJECT_ROOT, 'Demo', 'satellite_2d_dssl', 'augmentation_satellite.yml'),
                        help='量纲增强配置文件路径')
    # 数据增强参数
    parser.add_argument('--augment_dir', type=str, default='/data/wqn/datasets/packaged_dataset20251017_6c/')
    parser.add_argument('--augment_sample_limit', type=int, default=None)
    ...
    input_dim_mat_np = build_input_dim_matrix_from_yaml_v2(args.config_path)
    ...
    # 使用数据增强方式加载数据
    down = 4
    train_loader, valid_loader, aug_loader, x_normalizer, y_normalizer = prepare_augmented_dataloader(
        ...
        down=down,
        work_path=work_path,
        augment_sample_limit=args.augment_sample_limit,
        num_workers=args.num_workers,
        pin_memory=True
    )
    logger.info(f'数据加载完成: train={len(train_loader.dataset)}, valid={len(valid_loader.dataset)}, aug={len(aug_loader.dataset)}')
    ...
    for epoch in range(args.epochs):
        ...
        # 设置自监督学习率：前 400 epoch 为 0，之后按正常比例计算
        if epoch < 300:
            lr_self = 0.0
        else:
        ...
        self_loss = train_selfsup(
            ...
            output_dim_mat,
            Optimizer_self,
            skip_backward=(lr_self == 0.0),  # 学习率为 0 时跳过梯度计算
        )
