...
# 定义训练样本数量列表
NTRAIN_LIST=(200 800 1500 2500 5000)
...
for ntrain in "${NTRAIN_LIST[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        echo "[Trans_DSSL] ntrain=$ntrain 结束时间: $(date '+%Y-%m-%d %H:%M:%S') [成功]" | tee -a "$LOG_FILE"
    else
        echo "[Trans_DSSL] ntrain=$ntrain 结束时间: $(date '+%Y-%m-%d %H:%M:%S') [失败]" | tee -a "$LOG_FILE"
    fi
