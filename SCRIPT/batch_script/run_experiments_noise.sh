...
# 噪声训练实验脚本
# 测试不同样本数 (2000, 3000, 4000) 和噪声强度 (0.005, 0.01, 0.05, 0.1, 0.2)
...
for ntrain in "${NTRAIN_LIST[@]}"; do
    for noise in "${NOISE_STD_LIST[@]}"; do
        for script in "run_MLP_satellite_base_noise.py" \
            ...
            task_queue+=("$ntrain|$noise|$script")
        done
    done
done
...
run_task() {
    ...
    echo "[${model}_${type}] n=$ntrain, noise=$noise, CUDA=$cuda 开始: $(date)" | tee -a "$LOG_FILE"
    
    # 执行训练，输出重定向到单独的日志
    task_log="${LOG_FILE%.log}_${model}_${type}_n${ntrain}_noise${noise}.log"
    ...
    local status=$?
    if [ $status -eq 0 ]; then
        echo "[${model}_${type}] n=$ntrain, noise=$noise 完成 ✓ $(date)" | tee -a "$LOG_FILE"
    else
        echo "[${model}_${type}] n=$ntrain, noise=$noise 失败 ✗ $(date)" | tee -a "$LOG_FILE"
    fi
...
while [ $task_idx -lt ${#task_queue[@]} ] || [ ${#running_pids[@]} -gt 0 ]; do
    ...
    while [ ${#running_pids[@]} -lt $MAX_PARALLEL ] && [ $task_idx -lt ${#task_queue[@]} ]; do
        ...
        # 后台执行任务
        run_task "$ntrain" "$noise" "$script" "$cuda" &
        pid=$!
