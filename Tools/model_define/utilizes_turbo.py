...
def get_grid(real_path=None, GV_RB=False, grid_num=128):
    ...
    if GV_RB:
        hub_file = os.path.join('D:\WQN\CODE\DENO4pytorch-main\Demo\GV_RB\hub_lower_GV.txt')
        shroud_files = os.path.join('D:\WQN\CODE\DENO4pytorch-main\Demo\GV_RB\shroud_upper_GV.txt')
...
def get_origin_GVRB(quanlityList=None,
    ...
    if realpath is None:
        sample_files = [os.path.join("D:\WQN\CODE\DENO4pytorch-main\Demo\GV_RB\data\sampleALLnew_5000")]
    else:
        sample_files = [os.path.join( "D:\WQN\CODE\DENO4pytorch-main\Demo\GV_RB\data\sampleALLnew_5000")]
    if existcheck:
        sample_files_exists = []
...
def get_quanlity_from_GVRBmat(sample_files, quanlityList, invalid_idx=False):
    ...
    for ii, file in enumerate(sample_files):
        ...
        for jj, quanlity in enumerate(quanlityList):
             ...
             # elif quanlity == "DensityFlow":  # 设置一个需要计算获得的数据
             #     Vm = np.sqrt(np.power(reader.read_field("Vx"), 2) + np.power(reader.read_field("Vy"), 2))
             #     output[:, :, :, jj] = (reader.read_field("Density") * Vm).copy()
             elif quanlity == "Density":  # 设置一个需要计算获得的数据
                 output[:, :, :, jj] = (reader.read_field("Density")).copy()
             elif quanlity == "Vx":  # 设置一个需要计算获得的数据
                 output[:, :, :, jj] = (reader.read_field("Vx")).copy()
             elif quanlity == "Vy":  # 设置一个需要计算获得的数据
                 output[:, :, :, jj] = (reader.read_field("Vy")).copy()
             elif quanlity == "Vz":  # 设置一个需要计算获得的数据
                 output[:, :, :, jj] = (reader.read_field("Vz")).copy()
             elif quanlity == "Relative Total Temperature":  # 设置一个需要计算获得的数据
                 output[:, :, :, jj] = (reader.read_field("Relative Total Temperature")).copy()
...
class WeightLoss(torch.nn.Module):
    def __init__(self, zList=None, bar=0, weighted_cof=10):
    ...
    def get_weighted_mat(self, shape):
        temp = torch.zeros(shape)
        idx = []
        for z in self.zList:
            idx.append(range(max(z - self.bar, 0),min(z + self.bar, shape[1])))
        temp[:,idx,...] = temp[:,idx,...] + self.weighted_cof - 1
        temp = temp + 1
        weighted_mat = temp / np.mean(temp)
    ...
    def forward(self, predicted, target):
        ...
        weighted_mat = self.get_weighted_mat(self, target.shape[1:-1])
        weighted_mat = weighted_mat.unsqueeze(0).unsqueeze(-1).expand_as(target)
        weighted_mat = weighted_mat.to(device)
